...
- (void)activateWithAllowedSystemItems:(NSArray<NSNumber *> *)systemItems
    ...
    void (^finish)(NSError * _Nullable) = ^(NSError * _Nullable error) {
        ...
        @synchronized (self) {
            ...
            if (generation != self.generation && error == nil) {
                error = [self errorWithCode:5 description:@"The visibility request was cancelled."];
            }
    ...
    @try {
        ...
        // A changed beta API must not leave the UI waiting indefinitely.
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, 8 * NSEC_PER_SEC),
                       dispatch_get_main_queue(), ^{
            finish([self errorWithCode:6 description:@"The native menu-bar API timed out."]);
        });
