        ...
        return try window.children
            ...
            .map { group in
                guard let frame = group.frame, isUsable(frame) else {
                    throw MenuBarIconHidingError.menuBarReadFailed
                }
...
final class SystemAXMenuBarSource: AXMenuBarSource {
    ...
    /// Read only the selected menu bar's direct groups. Never traverse app
    /// menus recursively (those contain actions unrelated to status icons).
    private func shelfGroups() throws -> [AXUIElement] {
        guard AXIsProcessTrusted(),
              let agent = NSRunningApplication.runningApplications(
                withBundleIdentifier: "com.apple.MenuBarAgent").first else {
            throw MenuBarIconHidingError.accessibilityDenied
        }
        let app = AXUIElementCreateApplication(agent.processIdentifier)
        AXUIElementSetMessagingTimeout(app, 0.25)
        guard let windows: [AXUIElement] = attribute(kAXWindowsAttribute, from: app),
              let window = windows.first(where: {
                  frame(of: $0)?.intersects(preferredDisplayBounds()) == true
              }) else { throw MenuBarIconHidingError.menuBarReadFailed }
        // macOS 27 can expose these containers as AXUnknown instead of
        // AXGroup. Match makeWindowNode's normalization used by hiding;
        // identity and usable geometry are validated on each item below.
        let groups = directChildren(of: window)
        guard !groups.isEmpty else { throw MenuBarIconHidingError.menuBarReadFailed }
        return groups
    }

    func shelfTargets(hiddenBy allowed: MenuBarVisibleItems, markerX: CGFloat) throws -> [MenuBarShelfTarget] {
        try shelfGroups().compactMap { group in
            let node = makeItemGroupNode(from: group)
            guard let bounds = node.frame, bounds.width > 0, bounds.height > 0 else { return nil }
            let records = try? AXMenuBarRecordCollector.records(from: [
                .init(role: kAXWindowRole, frame: preferredDisplayBounds(),
                      bundleIdentifier: nil, systemItemIdentifier: nil, children: [node])
            ], preferredDisplayBounds: preferredDisplayBounds())
            guard let record = records?.first,
                  MenuBarShelfSupport.isHidden(bundle: record.bundleIdentifier,
                    system: record.systemItemIdentifier, midX: bounds.midX,
                    markerX: markerX, allowed: allowed) else { return nil }
            let app = record.bundleIdentifier.flatMap {
                NSRunningApplication.runningApplications(withBundleIdentifier: $0).first
            }
            let name = app?.localizedName ?? directChildren(of: group).compactMap {
                self.attribute(kAXDescriptionAttribute, from: $0) as String?
            }.first ?? "Menu bar item"
            let identity = record.bundleIdentifier ?? "system.\(record.systemItemIdentifier ?? -1)"
            return MenuBarShelfTarget(id: "\(identity).\(bounds.minX)",
                bundleIdentifier: record.bundleIdentifier, systemIdentifier: record.systemItemIdentifier,
                name: name, frame: bounds)
        }.sorted { $0.frame.minX < $1.frame.minX }
    }

    /// Resolve a fresh visible target by owner, then verify the AX frame.
    /// An ambiguous multi-icon owner is deliberately not clicked automatically.
    func pressShelfTarget(_ target: MenuBarShelfTarget) throws {
        let matches = try shelfGroups().filter { group in
            let node = makeItemGroupNode(from: group)
            if let bundle = target.bundleIdentifier {
                return node.bundleIdentifier == bundle || node.children.contains { $0.bundleIdentifier == bundle }
            }
            return node.children.contains { child in
                child.children.contains { $0.systemItemIdentifier == target.systemIdentifier && target.systemIdentifier != nil }
            }
        }
        guard matches.count == 1, let group = matches.first,
              let bounds = frame(of: group) else { throw MenuBarIconHidingError.menuBarReadFailed }
        var candidates = directChildren(of: group).filter { role(of: $0) == kAXButtonRole }
        for child in directChildren(of: group) where role(of: child) == kAXGroupRole {
            candidates += directChildren(of: child).filter { role(of: $0) == kAXMenuBarItemRole }
        }
        if let bundle = target.bundleIdentifier,
           let app = NSRunningApplication.runningApplications(withBundleIdentifier: bundle).first {
            let element = AXUIElementCreateApplication(app.processIdentifier)
            AXUIElementSetMessagingTimeout(element, 0.25)
            if let extras: AXUIElement = attribute("AXExtrasMenuBar", from: element) {
                candidates += directChildren(of: extras)
            }
        }
        guard let button = candidates.first(where: { candidate in
            guard let rect = frame(of: candidate) else { return false }
            return rect.insetBy(dx: -2, dy: -2).contains(CGPoint(x: bounds.midX, y: bounds.midY))
        }) else { throw MenuBarIconHidingError.menuBarReadFailed }
        AXUIElementSetMessagingTimeout(button, 0.25)
        let result = AXUIElementPerformAction(button, kAXPressAction as CFString)
        // NSMenu tracking can block the target's reply even when it opened.
        // Never retry or synthesize another click on cannotComplete.
